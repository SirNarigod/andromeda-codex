...
var _spatial_registry: AndromedaInteractionSpatialBindingRegistry
...
var _authority_approach_requests: int = 0
var _authority_approach_movement_commands: int = 0
var _invalidated_approaches_cancelled: int = 0
...
func bind_runtime(
	...
	spatial_registry_value: AndromedaInteractionSpatialBindingRegistry
) -> Dictionary:
	if (
		...
		or spatial_registry_value == null
	):
		return _rejected("MAIN_PLAYER_HUD_LOOT_COMBAT_ACTORS_AND_SPATIAL_REGISTRY_REQUIRED")
	...
	_spatial_registry = spatial_registry_value
	if not _spatial_registry.target_invalidated.is_connected(_on_spatial_target_invalidated):
		_spatial_registry.target_invalidated.connect(_on_spatial_target_invalidated)
	if not _bound:
		var bridge := _bridge()
		if bridge != null:
...
func begin_attack(target: AndromedaInteractionTarget, request_source: String) -> Dictionary:
	...
	if destination == Vector3.INF:
		return _rejected("AUTHORITATIVE_TARGET_SPATIAL_BINDING_NOT_READY")
	...
	_authority_approach_requests += 1
	...
	if movement.get("transport_submitted") == true:
		_authority_approach_movement_commands += 1
	...
	return {
		...
		"target_binding": _spatial_registry.binding_for_ref(target.target_ref),
...
func approach_destination_for(target: AndromedaInteractionTarget, attack_range_m: float) -> Vector3:
	if _spatial_registry == null:
		return Vector3.INF
	var binding: Dictionary = _spatial_registry.authoritative_target_local_position(target.target_ref)
	if binding.get("status") != "PASS" or not binding.get("local_position") is Vector3:
		return Vector3.INF
	var target_position: Vector3 = binding.get("local_position")
	var away := Vector2(
...
func project_combat_snapshot(snapshot: Dictionary) -> Dictionary:
	...
	for slot: String in ["COMMON", "ALPHA"]:
		...
		var spatial_binding: Dictionary = _spatial_registry.project_target(
			snapshot,
			entry,
			actor,
			"ACTOR"
		)
		if spatial_binding.get("status") != "PASS":
		...
		projected.append({
			...
			"target_ref": next_ref,
			"identity": identity,
			"local_position": spatial_binding.get("local_position"),
			"authority_position": spatial_binding.get("authority_position"),
...
func contract_snapshot() -> Dictionary:
	return {
		...
		"target_spatial_position_authority": false,
		"target_position_source": "INTERACTION_SPATIAL_BINDING_REGISTRY",
		"authority_approach_requests": _authority_approach_requests,
		"authority_approach_movement_commands": _authority_approach_movement_commands,
		"invalidated_approaches_cancelled": _invalidated_approaches_cancelled,
...
func _validate_target(target: AndromedaInteractionTarget) -> Dictionary:
	...
	if _spatial_registry == null:
		return _rejected("INTERACTION_SPATIAL_BINDING_REGISTRY_REQUIRED")
	var binding: Dictionary = _spatial_registry.binding_for_ref(target.target_ref)
...
func _physical_distance_to(target: AndromedaInteractionTarget) -> float:
	...
	if binding.get("status") != "PASS" or not binding.get("local_position") is Vector3:
		return INF
	return _player.global_position.distance_to(binding.get("local_position"))
...
func _on_spatial_target_invalidated(evidence: Dictionary) -> void:
	var target_ref_value: String = str(evidence.get("target_ref", "")).strip_edges()
	if target_ref_value.is_empty() or not _pending_approach.has(target_ref_value):
		return
	_pending_approach.erase(target_ref_value)
	_invalidated_approaches_cancelled += 1
	if _player != null:
...
func _reset_projection_caches() -> void:
	...
	_actors_by_ref.clear()
	if _spatial_registry != null:
		_spatial_registry.invalidate_all("COMBAT_CLIENT_SESSION_RESET", true)
	_last_request.clear()
