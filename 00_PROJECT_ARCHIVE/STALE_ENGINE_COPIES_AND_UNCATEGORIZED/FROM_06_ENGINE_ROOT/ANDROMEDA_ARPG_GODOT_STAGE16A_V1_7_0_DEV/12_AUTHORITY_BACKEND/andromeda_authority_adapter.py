...
class Stage16BAuthorityAdapter:
        ...
        for slot_name, rank in (("common", "COMMON"), ("alpha", "CHAMPION")):
            ...
            if stored and stored not in slot_excluded:
                try:
                    ...
                    # Reuse only a pristine actor. A stored target that already took
                    # damage in an earlier run would make the next encounter
                    # unreproducible, and healing it here would be an invented rule.
                    # Selecting a different untouched actor invents nothing.
                    if (
            ...
            if not target_ref:
                ...
                for npc in self._engine.country(self.world_instance_id)._all_npc_records():
                ...
                if not candidates:
                    raise AuthorityBootError("no eligible Stage07 combat encounter actor")
    ...
    def _dispatch(
        ...
        if command == "REQUEST_GATHER":
            return self._request_gather(params, command_ref)
    ...
    def _request_gather(
        ...
        """
        unexpected = sorted(set(params) - {"target_ref", "requested_units"})
        if unexpected:
            return self._rejected(
                "UNEXPECTED_COMMAND_PARAM", field_path=f"$.params.{unexpected[0]}"
            )
    ...
    def _confirm_drop_settled(
        ...
        reachable = params.get("reachable")
        if not drop_ref:
            return self._rejected("GROUND_LOOT_TARGET_REF_REQUIRED")
        if not isinstance(position, dict):
