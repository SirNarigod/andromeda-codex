...
func _run_gate() -> void:
	...
	var node_ref: String = _static_gathering_node(initial)
	_check("STATIC_GATHERING_NODE_AVAILABLE", not node_ref.is_empty(), node_ref)
...
func _static_gathering_node(snapshot: Dictionary) -> String:
	## A static resource activity always yields output, unlike HUNTING which can miss.
	var static_activities: Array[String] = ["LOGGING", "MINING", "FORAGING", "AGRICULTURE"]
	for value: Variant in snapshot.get("gathering_nodes", []):
		if not value is Dictionary:
			continue
		var entry: Dictionary = value
		if not static_activities.has(str(entry.get("activity_type", ""))):
			continue
...
func _create_authoritative_drop(bridge: AndromedaRuntimeBridge, node_ref: String, label: String) -> String:
	...
	for attempt: int in range(1, 9):
		...
		if result.is_empty():
			return ""
		var drop_ref: String = str(result.get("drop_ref", ""))
		if result.get("status") == "PASS" and not drop_ref.is_empty():
			return drop_ref
	return ""
