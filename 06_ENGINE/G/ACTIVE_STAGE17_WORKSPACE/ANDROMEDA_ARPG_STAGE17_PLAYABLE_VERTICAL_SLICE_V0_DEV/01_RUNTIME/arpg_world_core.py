...
def _iter_coord_pairs(node: Any):
    # Achado ao rodar os 13 territorios reais (14/09): geometry.type varia
    # (Polygon/MultiPolygon/LineString/MultiLineString/...), cada um com
    # coordinates aninhado num nivel diferente - assumir sempre Polygon
    # (coordinates[0] = lista plana de pontos) quebrava com TypeError pra
    # qualquer territorio com geometria nao-Polygon (ex.: TER-012). Desce
    # recursivamente ate achar um par [lon,lat] numerico, em vez de assumir
    # profundidade fixa.
    if isinstance(node, (list, tuple)):
        if len(node) >= 2 and all(isinstance(x, (int, float)) for x in node[:2]):
            ...
            return
        for child in node:
            yield from _iter_coord_pairs(child)


def _polygon_bbox(geometry: dict[str, Any]) -> tuple[float, float, float, float]:
    pairs = list(_iter_coord_pairs((geometry or {}).get("coordinates")))
    if not pairs:
...
class ARPGWorldCore:
    ...
    def __init__(self, engine: Any, world_instance_id: str) -> None:
        ...
        # Fase 8.3 (Stage17, 14/09): PILOT_TERRITORY_ID/NAME e os ids de bioma/
        # clima/hidrologia eram constantes de classe fixas em TER-011 - agora
        # sao auto-derivados do CountryScaleSystem ja anexado a este mundo
        # (country.country_territory_id, ja parametrizavel desde
        # IntegratedLivingEngineV12.create_world(..., country_territory_id=...)),
        # preservando 100% o comportamento de TER-011 (mesmo default da cadeia
        # inteira) e permitindo um 2o territorio real sem duplicar este core.
        self.PILOT_TERRITORY_ID = self.country.country_territory_id
        self._init_schema()
    ...
    def _validate_pilot_authority(self) -> None:
        # Fase 8.3: PILOT_TERRITORY_NAME/MAJOR_BIOME_ID/PRIMARY_BIOME_ID/CLIMATE_ID/
        # HYDROLOGY_IDS eram literais fixos de TER-011 - agora sao lidos do proprio
        # registro canonico do territorio configurado (self.PILOT_TERRITORY_ID),
        # preservando a mesma checagem de auto-consistencia/anti-drift (o
        # territorio ainda precisa bater exatamente com o que o canon diz que
        # ele e), so sem travar em TER-011 especificamente. Convencao observada
        # em >=2 territorios reais do canon (TER-011, TER-003): biome_ids e
        # sempre [bioma_major_regional, bioma_primario_local] nessa ordem.
        r = self._records()
        ter = r["territories"].get(self.PILOT_TERRITORY_ID)
        if not ter:
            raise IntegrityError(f"Stage08 pilot territory authority mismatch: {self.PILOT_TERRITORY_ID} not in canon")
        self.PILOT_TERRITORY_NAME = ter.get("name")
        # Achado ao rodar os 13 territorios reais (14/09): "sempre exatamente
        # 2 biome_ids" so era verdade por coincidencia em TER-011/TER-003 -
        # no canon real a contagem varia (1 pra 5 territorios, 2 pra 6, 3 pra
        # TER-001). Regra generalizada, preservando a semantica ja observada
        # (major_system = o bioma regional recorrente, sempre biome_ids[0]
        # quando ha mais de 1; primary = o bioma local, sempre biome_ids[-1]):
        # 1 bioma -> major e primary sao o mesmo (nao ha overlay regional
        # separado do local); 2+ biomas -> primeiro=major, ultimo=primary.
        biome_ids = list(ter.get("biome_ids", []))
        if not biome_ids:
            raise IntegrityError(f"Stage08 pilot biome authority mismatch: no biome_ids for {self.PILOT_TERRITORY_ID}")
        self.MAJOR_BIOME_ID = biome_ids[0]
        self.PRIMARY_BIOME_ID = biome_ids[-1]
        climate_ids = list(ter.get("climate_ids", []))
        if len(climate_ids) != 1:
            raise IntegrityError(f"Stage08 pilot climate authority mismatch: expected exactly 1 climate_id for {self.PILOT_TERRITORY_ID}, got {climate_ids}")
        self.CLIMATE_ID = climate_ids[0]
        self.HYDROLOGY_IDS = tuple(ter.get("hydrology_ids", []))
        if not self.HYDROLOGY_IDS:
            raise IntegrityError(f"Stage08 pilot hydrology authority mismatch: no hydrology_ids for {self.PILOT_TERRITORY_ID}")
        # Achado ao generalizar (14/09): para TER-011, geometry do bioma primario
        # bate EXATAMENTE com a do territorio (coincidencia dessa entrada
        # especifica do canon, confirmada olhando os dados reais). Pra TER-003
        # isso NAO vale (BIO-007 e um poligono real diferente do territorio,
        # so sobreposto) - exigir igualdade exata rejeitaria um territorio
        # canonico legitimo. Checagem generalizada: bbox do bioma primario
        # precisa sobrepor o bbox do territorio (ainda pega bioma errado/nao
        # relacionado, sem travar em a coincidencia geometrica exata de TER-011).
        primary = r["biomes"][self.PRIMARY_BIOME_ID]
        primary_geom = primary.get("geometry")
        territory_geom = ter.get("geometry")
        if not primary_geom or not territory_geom:
            raise IntegrityError(f"{self.PRIMARY_BIOME_ID}/{self.PILOT_TERRITORY_ID} missing canonical geometry")
        if not _bbox_overlaps(_polygon_bbox(primary_geom), _polygon_bbox(territory_geom)):
            raise IntegrityError(f"{self.PRIMARY_BIOME_ID} geometry does not overlap {self.PILOT_TERRITORY_ID} canonical geometry")
    ...
    def _canonical_start_location_id(self) -> str | None:
        # Fase 8.3: "CIT-001" era o inicio canonico fixo de TER-011 - agora
        # derivado do proprio ter["major_location_ids"] do territorio
        # configurado (a cidade/CIT-* listada, ja que toda territory record
        # tem sua propria cidade principal como ponto de entrada canonico;
        # confirmado real pra TER-011=CIT-001 e TER-003=CIT-002).
        # Achado ao rodar os 13 territorios reais (14/09): so TER-011/TER-003
        # tem uma entrada CIT-* em major_location_ids - os outros 11 so tem
        # LOC-* (nenhuma cidade canonica catalogada pra eles ainda). Cair pra
        # a primeira location listada (seja CIT ou LOC) preserva a mesma
        # ideia - "o local principal do territorio e o ponto de entrada" -
        # sem exigir uma cidade que nao existe no canon pra esses territorios.
        ter = self._records()["territories"][self.PILOT_TERRITORY_ID]
        location_ids = list(ter.get("major_location_ids", []))
        cities = [lid for lid in location_ids if str(lid).startswith("CIT-")]
        if cities:
            return cities[0]
        return location_ids[0] if location_ids else None

    def _find_start_zone(self) -> tuple[str, str | None]:
        start_location_id = self._canonical_start_location_id()
        if start_location_id:
            target_wp = "WP-CANON-" + start_location_id
            for z in self.list_zones():
                for wp in z.get("waypoints", []):
                    if wp["waypoint_ref"] == target_wp:
    ...
    def verify(self) -> dict[str, Any]:
        ...
        # Fase 8.3: a tupla fixa ("RTE-001","RTE-003","RTE-013") era literalmente
        # ter["route_ids"] de TER-011 (confirmado no canon real) - generalizado
        # pra iterar as rotas do PROPRIO territorio configurado, so exigindo
        # lock em quem de fato cruza fronteira (BOUNDARY_GATE_ONLY); uma rota
        # totalmente interna de outro territorio nao deve ser reprovada por
        # nao estar travada (ela nunca deveria estar).
        ter_route_ids = self._records()["territories"][self.PILOT_TERRITORY_ID].get("route_ids", [])
        for rid in ter_route_ids:
            rs = self.boundary_route(rid)
            if rs.get("playability") == "BOUNDARY_GATE_ONLY" and not rs.get("boundary_locked"):
                failures.append("PILOT_OUTBOUND_ROUTE_NOT_LOCKED:" + rid)
        start_zone, start_wp = self._find_start_zone()
        expected_start_wp = "WP-CANON-" + (self._canonical_start_location_id() or "")
        if not start_zone or start_wp != expected_start_wp:
