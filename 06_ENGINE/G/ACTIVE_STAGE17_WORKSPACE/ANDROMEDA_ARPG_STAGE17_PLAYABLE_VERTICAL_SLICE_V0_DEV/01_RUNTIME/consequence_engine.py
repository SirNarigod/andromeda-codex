    ...
    def _materialize_templates(
        ...
        for i, t in enumerate(rule["consequences"]):
            ...
            if "amount_from_event" in c:
                ...
                amount *= multiplier
                # S17: lets one rule apply a DIFFERENT weight per matched
                # relation (e.g. "each present faction reacts scaled by ITS
                # OWN influence") -- previously every target under a rule
                # got the exact same event-sourced amount, which is why the
                # kill/death->faction-reputation bridges could not be
                # expressed as a declarative rule and stayed composed
                # functions (see ANDROMEDA_CAUSAL_RULES_CATALOG_V1_0.md).
                # `weight_from_relation_metadata` names a key in the
                # MATCHED relation's own metadata (register_relation(...,
                # metadata={"weight": 0.8})); missing/absent means weight 1
                # (fully backward compatible -- a relation with no metadata
                # weight behaves exactly as before this change).
                weight_key = c.pop("weight_from_relation_metadata", None)
                if weight_key is not None:
                    if relation is None:
                        raise ValidationError("weight_from_relation_metadata requires a matched relation")
                    metadata = json.loads(relation["metadata_json"]) if relation["metadata_json"] else {}
                    weight = metadata.get(weight_key, 1)
                    if not isinstance(weight, (int, float)) or isinstance(weight, bool):
                        raise ValidationError(f"relation metadata weight '{weight_key}' is not numeric")
                    amount *= weight
                if amount < 0:
                        ...
                        for subject_ref in event.subjects:
                            ...
                            for candidate in candidates:
                                ...
                                if allowed_kinds and target.get("entity_kind") not in allowed_kinds:
                                    continue
                                try:
                                    consequences = self._materialize_templates(event, target_ref, rule, relation)
                                except Exception as exc:
                                    self._record_stop(
