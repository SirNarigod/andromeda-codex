...
class ARPGMobilityInfrastructureCore:
    ...
    def __init__(self, engine: Any, world_instance_id: str) -> None:
        ...
        # Fase 8.6 (Stage17, 14/09): COUNTRY_REF/BOUNDARY_ROUTES/CANON_LOCAL_ROAD_REF/
        # CANON_BRIDGE_REF eram literais fixos de TER-011 - generalizados pra
        # qualquer um dos 13 territorios reais do canon, derivados de
        # self.country.country_territory_id (mesmo padrao ja usado em
        # arpg_world_core.py). CANON_LOCAL_ROAD_REF/CANON_BRIDGE_REF podem
        # nao existir pro territorio (achado real: so 9/13 tem estrada local
        # SRTE-*, so 4/13 tem ponte canonica) - viram None e o bootstrap
        # correspondente e pulado, em vez de forcar um dado que nao existe.
        self.COUNTRY_REF = self.country.country_territory_id
    ...
    def _load_canon(self):
        ...
        territory_path='ANDROMEDA_CODEX_MASTER/03_DOMAINS/GEOSPATIAL/02_GEOGRAFIA/STELLAR_TERRITORIES_V1_0.json'
        with zipfile.ZipFile(p) as z:
            master=json.loads(z.read(entity_path)); skeleton=json.loads(z.read(route_path))
            routeidx=json.loads(z.read(route_idx_path)); territories=json.loads(z.read(territory_path))
        ...
        ter={t.get('id'):t for t in territories.get('territories',[]) if isinstance(t,dict)}.get(self.COUNTRY_REF)
        if not ter: raise IntegrityError('Stage10 territory not found in canon:'+self.COUNTRY_REF)
        # Rotas de fronteira do PROPRIO territorio (ter['route_ids']), nao
        # mais literal fixo de TER-011 - isto SIM precisa variar por
        # territorio (e o que determina quais fronteiras cada territorio
        # pode desbloquear).
        self.BOUNDARY_ROUTES=tuple(ter.get('route_ids') or [])
        # CANON_LOCAL_ROAD_REF/CANON_BRIDGE_REF (SRTE-001/POI-046) PERMANECEM
        # fixos (classe), nao derivados por territorio - achado ao testar ao
        # vivo: sao ancoras GLOBAIS com consumidores hardcoded em outros 3
        # arquivos (arpg_economy_crafting_core.py#L193 chama
        # self.mobility.bridge('POI-046') incondicionalmente,
        # arpg_living_integration_core.py#BRIDGE_REF,
        # arpg_narrative_culture_core.py#LOCAL_ANCHORS/quest "Continuidade da
        # Ponte Varden") - tentar torna-las por-territorio quebrava esses 3
        # subsistemas pra qualquer territorio != TER-011/001/003 (regressao
        # real, confirmada ao vivo, corrigida aqui revertendo pra fixo).
    ...
    def _bootstrap_segments(self):
        # Canonical local Varga road (ancora global fixa - ver nota em _load_canon).
    ...
    def _bootstrap_bridge(self):
        ...
        # Ancora global fixa (ver nota em _load_canon) - sempre bootstrapada,
        # mesmo quando este territorio nao e TER-011 (outros subsistemas
        # dependem dela existir incondicionalmente).
    ...
    # Fase 7.2 (Stage17, PLANET_RUNTIME_NOT_CANON): desbloqueio incremental de fronteira.
    # Este pilot Stage10 e escopado a 1 pais (COUNTRY_REF=TER-011) por design - este metodo
    # NAO transforma a malha viaria em multi-pais, so permite que o orquestrador de planeta
    # (planet_runtime.py) libere UMA rota de fronteira especifica, uma vez que o territorio
    # de destino real foi confirmado geograficamente (via STELLAR_ENTITY_LOCATION_BINDINGS,
    # fora deste arquivo). Hoje so RTE-003->TER-003 tem essa confirmacao; RTE-001/RTE-013
    # permanecem travadas ate seus destinos serem confirmados numa fase futura.
    # Fase 8.6 (Stage17, 14/09): mapa completo das 21 rotas cross-territorio
    # confirmadas (ambos os lados) via STELLAR_ROUTE_SKELETON_V1_0.json x
    # STELLAR_TERRITORIES_V1_0.json.major_location_ids (25 rotas no total no
    # canon; 4 sao internas ao mesmo territorio, sobram 21 - grafo conexo,
    # liga os 13 territorios). Cada rota agora mapeia os 2 territorios que ela
    # liga (nao so 1 destino fixo) - unlock_boundary_route aceita qualquer um
    # dos 2 como destino, desde que diferente do territorio atual.
    ...
    def unlock_boundary_route(self,route_ref,destination_country_ref,*,authority='ARPG_STAGE17_PLANET_RUNTIME_NOT_CANON'):
        if route_ref not in self.BOUNDARY_ROUTES: raise ValidationError(f'{route_ref} is not a boundary route')
        endpoints=self._CONFIRMED_ROUTE_TERRITORIES.get(route_ref)
        if endpoints is None or destination_country_ref not in endpoints or destination_country_ref==self.COUNTRY_REF:
            return {'status':'REJECTED','reason':'DESTINATION_TERRITORY_NOT_CONFIRMED','route_ref':route_ref,'requested_destination':destination_country_ref}
        s=self.segment(route_ref); s['boundary_locked']=False; s['unlock_authority']=authority; s['unlocked_destination_country_ref']=destination_country_ref
        self._save_segment(s)
        return {'status':'PASS','route_ref':route_ref,'destination_country_ref':destination_country_ref,'authority':authority}
    ...
    def verify(self):
        ...
        # SYS-AUT-001 (o sistema automotivo canonico compartilhado por todos
        # os territorios) tem territory_refs=['TER-011'] como fato fixo da
        # PROPRIA entidade - e onde foi primeiro descrito no canon, nao uma
        # trava de "so TER-011 pode ter veiculos". Checagem generalizada:
        # confirma que o snapshot bate com esse fato fixo conhecido (anti-
        # drift), nunca mais comparado contra self.COUNTRY_REF (que agora
        # varia por territorio).
        if self.canonical_snapshot(self.CANON_AUTOMOTIVE_REF).get('territory_refs')!=['TER-011']: failures.append('AUTOMOTIVE_CANON_SCOPE')
