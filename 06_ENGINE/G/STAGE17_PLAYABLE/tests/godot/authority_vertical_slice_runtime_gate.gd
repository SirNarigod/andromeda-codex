...
func _run_gate() -> void:
	...
	# The combat target rotates live (see combat_authority_client.gd's
	# project_combat_snapshot(), which already rebinds combat_enemy_common/alpha to
	# the backend's current combat_target_common/alpha_ref every snapshot -- this
	# gate must never assume the COMMON slot still sits where the original fixture
	# actor was placed once that actor is defeated and the slot rotates to a
	# different, live NPC elsewhere in the zone). Approach the *current* live
	# authoritative position from the same snapshot before ever attacking, the same
	# way every other interaction in this vertical slice (dialogue, gather, loot)
	# approaches its real target instead of assuming proximity.
	var combat_snapshot: Dictionary = await _fresh_snapshot(bridge)
	var enemy_position: Dictionary = {}
	var enemy_attack_range_m: float = 0.0
	for target_value: Variant in combat_snapshot.get("combat_targets", []):
		if target_value is Dictionary and str((target_value as Dictionary).get("target_ref", "")) == enemy_target_ref:
			enemy_position = (target_value as Dictionary).get("position", {})
			enemy_attack_range_m = float((target_value as Dictionary).get("attack_range_m", 0.0))
			break
	_check("G19_COMBAT_TARGET_POSITION_FROM_LIVE_SNAPSHOT", enemy_position.has("iso_x_m") and enemy_attack_range_m > 0.0, JSON.stringify(enemy_position))
	if enemy_position.has("iso_x_m"):
		# The rotated COMMON candidate can legitimately land far from spawn once the
		# nearby pristine pool is exhausted by earlier gates in the same long-lived
		# world, so approach in real authoritative steps and keep submitting until
		# the server itself reports arrival, the same retry-until-arrived shape
		# already used below for the LOOT approach, rather than a single fixed
		# duration_s that only covers a short, assumed-nearby hop.
		for step: int in range(1, 41):
			var approach_move: Dictionary = await _await_command(bridge, "MOVE_TO_POINT", {
				"iso_x_m": float(enemy_position.get("iso_x_m", 0.0)),
				"iso_y_m": float(enemy_position.get("iso_y_m", 0.0)),
				"duration_s": 10.0,
				"mode": "WALK",
			}, "SLICE-COMBAT-APPROACH-%d" % step)
			if approach_move.get("status") != "PASS" or bool(approach_move.get("arrived", false)):
				break
	...
	var gather_resource: Dictionary = {}
	for resource_value: Variant in gather_snapshot.get("resources", []):
		if resource_value is Dictionary and str((resource_value as Dictionary).get("state", "")) == "AVAILABLE":
			gather_resource = (resource_value as Dictionary).duplicate(true)
			break
	var gather_placement_ref: String = str(gather_resource.get("placement_ref", ""))
	var gather_node_ref: String = str(gather_resource.get("logical_node_ref", ""))
	var gather_position: Dictionary = gather_resource.get("position", {}) as Dictionary
	_check(
		"G19_GATHER_NODE_FOUND_AUTHORITATIVELY",
		not gather_placement_ref.is_empty() and not gather_node_ref.is_empty(),
		JSON.stringify(gather_snapshot.get("resources", []))
	)
	if gather_placement_ref.is_empty() or gather_node_ref.is_empty():
		await _finish()
		return
	var gather_move: Dictionary = await _await_command(bridge, "MOVE_TO_POINT", {
		"iso_x_m": float(gather_position.get("iso_x_m", 0.0)),
		"iso_y_m": float(gather_position.get("iso_y_m", 0.0)),
		"duration_s": 10.0,
		"mode": "WALK",
	}, "SLICE-GATHER-APPROACH")
	_check("G19_GATHER_METRIC_APPROACH_AUTHORITATIVE", gather_move.get("status") == "PASS", JSON.stringify(gather_move))
	var gather_result: Dictionary = await _await_command(bridge, "REQUEST_GATHER", {
		"placement_ref": gather_placement_ref,
		"target_ref": gather_node_ref,
		"requested_units": 1,
	}, "SLICE-GATHER")
	...
	_check("G19_GATHER_METRIC_GUARD_REAL", gather_result.get("metric_spatial_guard") is Dictionary and float((gather_result.get("metric_spatial_guard", {}) as Dictionary).get("distance_m", INF)) <= 1.5, JSON.stringify(gather_result))
	...
	_progress("GATHER", {"placement_ref": gather_placement_ref, "node_ref": gather_node_ref, "drop_ref": gather_drop_ref, "node_after": gather_node_after})
